...
**One case the hold does not cover.** It treats a paused or disabled legacy Skyhook as not-in-flight, deliberately, so migration does not force a user to unpause. But pre-Jobs pause never stopped a *running* pod — it only blocked new scheduling. A Skyhook paused just before the upgrade can therefore still have a live raw pod, and unpausing it on the new operator before that pod finishes puts a Job alongside it on the same host `copyDir`. Nothing goes wrong while it stays paused (the new operator suspends and skips it, creating no Jobs). Do not read the agent's flag files as making that safe: they make *re-execution* idempotent and are not a lock, so they neither serialize two executors nor order concurrent writes into the shared `copyDir`. **The sequence is therefore unsupported rather than guarded** — the migration guide tells users to wait for a paused Skyhook's pre-upgrade pods to disappear before unpausing, and leaving it paused is safe indefinitely. Guarding it in code instead would mean teaching the in-flight gate to count legacy raw pods so a Job is never created beside one; that was weighed and declined, because the whole point of shipping the rename and the Jobs migration together is that the two execution models do not have to coexist, and one narrow, documented, opt-in sequence did not justify carrying legacy-pod awareness into the gate for a release.
