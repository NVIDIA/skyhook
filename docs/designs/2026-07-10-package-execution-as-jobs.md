...
| Stage bound | per-attempt `activeDeadlineSeconds` (new `stageTimeout` field), then park on a spent retry budget | hung/unpullable stages surface as `erroring` instead of hanging forever |
...
spec:
  ...
  activeDeadlineSeconds: <ceiling>                # whole-budget backstop; interrupt Jobs: <stageTimeout>
...
**Retries exhausted** (`BackoffLimitExceeded`, on genuine evidence) is what parks. The operator records `erroring` and the Failed Job stays put: the finished-Job rules deliberately do not delete it while the package's entry sits at (same stage, `erroring`), so nothing recreates the stage and the churn stops. The park clears on any explicit signal — `package rerun`/`reset`, a config update, a spec change, or failure-TTL expiry — after which the still-`erroring` state drives one fresh attempt: a deliberate slow-retry cadence until a human intervenes.

**The budget bounds every failure class, not just timeouts.** A crash-looping package now parks after `backoffLimit + 1` attempts — about 70 seconds at the default, paced by the Job controller's 10s/20s/40s backoff — where an unbounded limit let it retry for the whole `stageTimeout`. Packages that ride out transient environment flakiness (a registry blip, a mount not up yet) lose that hour of self-healing, which is why the limit is an operator knob (`JOB_BACKOFF_LIMIT`, chart value `controllerManager.manager.env.jobBackoffLimit`, default 3) rather than a constant.

**The Job-level ceiling** stays for the one case a per-attempt clock cannot bound: that clock runs from `pod.Status.StartTime`, which a pod the kubelet never acknowledges never gets, so a Job whose pods never start would sit Active forever. The Job clock runs from the Job's own `startTime` and does bound it. It is derived, not configured — `(backoffLimit + 1) × (stageTimeout + gracefulShutdown + 10m)` — and deliberately generous: it counts a full backoff gap and shutdown window for every attempt, one more gap than can actually elapse. Tightness buys nothing, while a ceiling *below* the retry budget would silently truncate it into a `DeadlineExceeded` that reads as a hang. `gracefulShutdown` is in the formula because `podReplacementPolicy: Failed` waits out every shutdown before replacing. The product is clamped, since three independently settable values can overflow into a negative the apiserver rejects outright.
...
So the contract is: the new value applies at the package's next stage. To apply it to work already under way, clear the Job — `kubectl nodewright package rerun <package>`, or delete the Job — and the stage restarts under the new value. The Job-level ceiling *is* mutable and could be patched in place, but on its own that would leave the ceiling and the per-attempt bound disagreeing, so it is left alone too.
...
The snapshot is skipped silently on any error and never delays the erroring/park path. It is taken only while the Job sits at `FailureTarget`, and there is no retry once the Job goes terminal: by then the Job controller has deleted the pod the logs live on, so there is nothing left to read. Missing that window (an operator restart, a leadership change) means the parked tombstone carries no log tail, which is the accepted cost of best-effort evidence. Pod logs are not readable through the controller-runtime client, so this adds a small client-go clientset and a `pods/log` `get` RBAC rule. On an **unreachable node** the Job can sit at `FailureTarget` without ever going terminal, so a stale `FailureTarget` is treated as erroring evidence too (state write only; the marker/TTL/park still wait for `Failed`), and a reboot interrupt that bricks its node still surfaces.
...
- **The deadlines pause with the Job.** Suspension clears the Job's start time, so the Job-level ceiling stops ticking while paused, and it deletes the running pod, so the resumed attempt starts a fresh per-attempt clock — otherwise a "paused" stage could quietly hit a deadline and park while the user believed everything was frozen. Suspend-deletions are excluded from the failure count upstream (verified empirically), so pausing never spends the retry budget; `status.failed` is *not* reset either, so pausing to investigate a half-failed stage and resuming leaves the remaining budget unchanged.
...
| A Job-level deadline as the *only* stage bound (the earlier draft) | `JobSpec.activeDeadlineSeconds` is terminal: exceeding it fails the Job permanently with no replacement pod, so the first expiry parked. That makes a timeout the one failure class with no retry, and it deletes the running pod, leaving only a 16KB annotation where every other failure keeps a full-log archive. Moving the bound to the pod template makes a timeout an ordinary retryable failure; the Job-level value survives only as a ceiling for pods that never start. |
| A separate option for the Job-level ceiling | A fourth Job knob to document, for a value with no independent meaning — and one an operator can set below `(backoffLimit+1) × stageTimeout`, silently truncating the retry budget into a `DeadlineExceeded` that reads as a hang. Deriving it makes that unrepresentable. |
...
**Two writers, one park predicate.** The park is the pair (terminal `Failed` Job, entry at (stage, `erroring`)) — nothing reads a stored verdict, so a lost classification cannot by itself lose a park. Two independent paths put the entry at `erroring`, using the same classification so they agree: the Pod watch writes it live, while the failed attempt still exists, and the terminal Job path writes it from the retained archives. Each covers the other's blind spot — the Pod watch survives terminated-pod GC reaping the archives, and the Job path survives the operator being down for the whole retry window. Both must miss to lose a park, and the consequence is bounded: the Job is swept, the stage re-runs, and the next cycle's evidence parks it. That is one extra retry cycle, not churn.
