...
**One case the hold does not cover.** It treats a paused or disabled legacy Skyhook as not-in-flight, deliberately, so migration does not force a user to unpause. But pre-Jobs pause never stopped a *running* pod — it only blocked new scheduling. A Skyhook paused just before the upgrade can therefore still have a live raw pod, and unpausing it on the new operator before that pod finishes puts a Job alongside it on the same host `copyDir`. Nothing goes wrong while it stays paused (the new operator suspends and skips it, creating no Jobs). The window is narrow and the agent's flag files keep re-execution idempotent, but it is the one path where the two models can overlap.
