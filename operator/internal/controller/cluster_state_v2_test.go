...
var _ = Describe("CleanupRemovedNodes", func() {
	It("should cleanup removed nodes from all status maps", func() {
		...
		Expect(mockSkyhook.Status.ConfigUpdates).To(HaveKey("node1"))
		Expect(mockSkyhook.Status.ConfigUpdates).To(HaveKey("node2"))
		Expect(mockSkyhook.Status.ConfigUpdates).NotTo(HaveKey("removed-node"))
