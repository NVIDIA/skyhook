...
var _ = Describe("NoOp", func() {
	It("uses the no_op wire type", func() {
		Expect(NoOp{}.Type()).To(Equal(NoOpType))
	})

	It("returns success without running a command", func() {
		status, err := NoOp{}.Run(context.Background(), newInterruptRunConfig())
	...
	It("validates context and execution configuration", func() {
		...
		status, err := NoOp{}.Run(nilContext, newInterruptRunConfig())
		Expect(status).To(Equal(execution.StatusFailed))
		Expect(err).To(MatchError("running interrupt: context is nil"))

		status, err = NoOp{}.Run(context.Background(), execution.Config{})
		Expect(status).To(Equal(execution.StatusFailed))
