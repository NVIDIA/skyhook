...
var _ = Describe("RegularStep.applyDefaults", func() {
	...
	It("does not touch OnHost (it has no absent state in Go)", func() {
		s := RegularStep{path: "foo.sh"}
		s.applyDefaults()
		Expect(s.OnHost()).To(BeFalse(), "applyDefaults must leave OnHost alone; Decode applies OnHost=true via its *bool overlay")
	})
})
...
var _ = Describe("RegularStep.Encode", func() {
	...
	It("rejects an explicitly invalid idempotence value", func() {
		...
		_, err := s.Encode()
		Expect(err).To(MatchError("bogus is not a valid idempotence value"))
	})
})
...
var _ = Describe("NewRegularStep", func() {
	...
	It("round-trips through Encode/Decode", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(round.Path()).To(Equal("foo.sh"))
		Expect(round.Idempotence()).To(Equal(Disabled))
	})
})
