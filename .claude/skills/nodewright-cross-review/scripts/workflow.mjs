...
- Status / State / Stage are three DISTINCT concepts, deliberately named to look alike: Status = health of a NodeWright CR or a node; State = one package's outcome on one node; Stage = which lifecycle phase that package is in. A field, variable, log key or doc line named for one that carries another IS a real finding. Authoritative definitions: docs/architecture/operator-status.md at the pinned commit.
...
2. Check, in this order of likelihood for this repo:
   ...
   - .github/workflows/, Makefile and *.mk, testdata/, and docs/ (including docs/user-guide/cli.md's compatibility matrix and any docs/ page the change makes false).
