...
uninstall pod errors, the NodeWright gets an `UninstallFailed` condition, and
...
2. Flip `uninstall.apply: true`. Assert:
   ...
   - NodeWright has condition `UninstallFailed`
...
4. Assert `nodeState_<name>: '{}'` (package absent — uninstall succeeded)
   and NodeWright is `status: complete` (the `UninstallFailed` condition is
   cleared).
...
- `nodewright.yaml` — Initial NodeWright with a broken `uninstall.sh`
